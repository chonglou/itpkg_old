...
class WikiHandler(BaseHandler):
        ...
        if name:
            ...
            if wiki:
                if self.__can_edit(wiki):
                    ...
                    self.render_message_widget(Message(messages=["你没有编辑[%s]页的权限" % name]))
            else:
                self.render_form_widget(form=form)
                ...
                if fm.validate():
                    ...
                    if wiki:
                        if self.__can_edit(wiki):
                            ...
                            self.render_message_widget(Message(ok=True, goto="/wiki/" + name))
                            return
                        else:
    ...
    def delete(self, name=None):
        ...
        if name:
            wiki = WikiDao.get_wiki(name=name)
            msg.goto = "/wiki/%s"%name
            if wiki:
                if self.__can_edit(wiki):
                    WikiDao.del_wiki(name)
                    msg.ok = True
                else:
                    msg.messages.append("你没有删除[%s]的权限" % name)
            else:
                msg.messages.append("知识库[%s]不存在" % name)
            self.render_message_widget(msg=msg)
